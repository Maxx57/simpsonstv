...
# VLC RC interface socket path
SOCKET_PATH = "/tmp/vlcsocket"


def SendVLC(msg: str):
    logging.info("VLC command: %s", msg)
    msg += "\n"
    try:
        ...
        # VLC RC doesn't send meaningful responses for most commands
        time.sleep(0.1)  # Brief pause to let command process
        client.close()
    except socket.timeout:
        logging.error(f"Socket timeout connecting to {SOCKET_PATH}")
    except FileNotFoundError:
        logging.error(f"VLC socket not found at {SOCKET_PATH}. Is VLC running with RC interface?")
    except Exception as e:
        logging.error(f"Error communicating with VLC: {e}")


# Commands here: https://wiki.videolan.org/Documentation:Streaming_HowTo/Command_Line_Interface/
def Act(x: int, y: int, delta_x: int, delta_y: int):
    ...
    # Left touch
        ...
        SendVLC(f"seek -{SEEK_SECS}")
    # Right touch (this was "Middle touch" but positioned on right)
    elif x > MAX_X - X_MARGIN:
        SendVLC(f"seek +{SEEK_SECS}")
    # Middle touch (this was "Right touch" but in the middle)
    else:
        SendVLC("pause")
